...
__gshared TickDuration maxPauseTime;
__gshared size_t countCollections;
...
class GC
{
    ...
    private void *mallocNoSync(size_t size, uint bits = 0, size_t *alloc_size = null, const TypeInfo ti = null) nothrow
    {
        ...
        debug(PRINTF_MALLOC) printf("GC::malloc(size = %d, gcx = %p)\n", size, gcx);
        assert(gcx);
        ...
        version(BACK_GC)
            if (gcx.canSweep && !gcx.bgCollecting)
                gcx.fullcollectFinish();
        ...
        if (bin < B_PAGE)
        {
            ...
            auto buckets = gcx.bucket[bucketIndex(noscan)].ptr;

            while (!buckets[bin] && !gcx.allocPage(bin, noscan))
            {
                switch (state)
                {
                case 0:
            ...
            debug(PRINTF_MALLOC) printf("\tmalloc => %p\n", p);
            debug(MEMSTOMP) memset(p, 0xF0, size);
            assert(pool.freebits.test(cast(size_t)(p - pool.baseAddr) >> pool.shiftBy));
            pool.freebits.clear(cast(size_t)(p - pool.baseAddr) >> pool.shiftBy);
        }
        else
        {
            p = gcx.bigAlloc(size, &pool, alloc_size, noscan);
            if (!p)
                onOutOfMemoryError();
        }
        debug (SENTINEL)
    ...
    private void *reallocNoSync(void *p, size_t size, uint bits = 0, size_t *alloc_size = null, const TypeInfo ti = null) nothrow
    {
        ...
        {   void *p2;
            ...
            debug(PRINTF_MALLOC) printf("GC::realloc(p = %p, size = %zu)\n", p, size);
            debug (SENTINEL)
            {
    ...
    body
    {
        ...
        debug(PRINTF_MALLOC) printf("GC::extend(p = %p, minsize = %zu, maxsize = %zu)\n", p, minsize, maxsize);
        debug (SENTINEL)
        {
            return 0;
        }
    ...
    {
        debug(PRINTF_MALLOC) printf("Freeing %p\n", cast(size_t) p);
        assert (p);
        ...
        {   // Add to free list
            ...
            pool.freebits.set(cast(size_t)(p - pool.baseAddr) >> pool.shiftBy);
        }
...
struct Gcx
{
    ...
    List*[B_MAX][NUM_BUCKETS] bucket;        // free list for each size
    ...
    void Dtor()
    {
        if (GC.config.profile)
        {
            ...
            printf("\tMaximum pause time:  %lld milliseconds\n",
                   maxPauseTime.to!("msecs", long));
            long gctime = (recoverTime + sweepTime + markTime + prepTime + collectRootsTime).to!("msecs", long);
            ...
            version(GETPROCESSMEMORYINFO)
            {
                ...
                printf("maxPoolMemory = %4lld MB,%5lld GC%5lld ms <%4lld ms\n",
                       cast(long)maxPoolMemory >> 20, cast(long)countCollections, gctime, maxPauseTime.to!("msecs", long));
        }
        ...
        foreach (pool; pooltable[0 .. npools])
        {
            ...
            if (pool.isLargeObject)
            {
                foreach (ref pn; 0 .. pool.npages)
                {
                    ...
                    debug(COLLECTELEM_PRINTF) printf("\tcollecting big %p\n", p);
            ...
            else
            {
                foreach (ref pn; 0 .. pool.npages)
                {
                    ...
                    for (; p < ptop; p += size, biti += bitstride, clearIndex += bitstride)
                    {
                        ...
                        debug(COLLECTELEM_PRINTF) printf("\tcollecting %p\n", p);
                        log_free(sentinel_add(p));
     ...
     */
    Pool *findPool(bool bypassCache = !USE_CACHE, bool bypassMinMax = false)(void *p) nothrow
    {
        ...
        if (bypassMinMax || (p >= _minAddr && p < _maxAddr))
        {
            if (_npools <= 1)
            {
     ...
     */
    void *bigAlloc(size_t size, Pool **poolPtr, size_t *alloc_size, bool noscan) nothrow
    {
        ...
        for (state = disabled ? 1 : 0; ; )
        {
            ...
            case 0:
                ...
                minimize();
                // Allocate new pool
                pool = newPool(npages, true, noscan);
                if (!pool)
                {   state = 2;
                    continue;
                }
     ...
     *  0       failed
     */
    int allocPage(Bins bin, bool noscan) nothrow
    {
        Pool*  pool;
        size_t n;
        size_t pn;
    ...
    void mark(void *pbot, void *ptop, int nRecurse) nothrow
    {
        ...
        for (; p1 < p2; p1++)
        {
            ...
            if (p >= _minAddr && p < _maxAddr)
            {
                ...
                auto pool = findPool!(true, true)(p);
                if (pool)
                {
                    size_t offset = cast(size_t)(p - pool.baseAddr);
                    size_t biti = void;
    ...
    */
    ...
    void markHeap() nothrow
    {
        ...
        while (anychanges)
        {
            ...
            debug(COLLECTELEM_PRINTF) printf("\t\tpass\n");
            anychanges = 0;
            for (n = 0; n < npools; n++)
            {
                pool = pooltable[n];
                 ...
                 Lnotfree:
                    ...
                    {   biti = bitbase + u / 16;
                        ...
                        {   List *list;
                            ...
                            list = cast(List *)(p + u);
                            if (list.next != buckets[bin])       // avoid unnecessary writes
                                list.next = buckets[bin];
                            list.pool = pool;
                            buckets[bin] = list;
                        }
